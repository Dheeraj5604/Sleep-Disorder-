...
weights = torch.tensor([3.0] * num_classes).to(device) 

none_idx = list(le_target.classes_).index('None')
weights[none_idx] = 1.0
...
print(f"Detected Classes: {le_target.classes_}")

print(f"Training Transformer on {len(X_train)} samples using device: {device}...")
...
print("Complete. Weights saved for 30k dataset.")



def evaluate_model(model, loader, device,classes=le_target.classes_):
    print("\n" + "="*50)
